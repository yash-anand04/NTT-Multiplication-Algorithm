...
module ntt_top #(
    ...
    wire [LOGR-1:0]   seq_bank = seq_cnt[LOGR-1:0];       // lower bits = bank
    ...
    // =========================================================================
    // NTT PATH:  D1→Norm → ModMul (twiddle) → Norm→D1 → R2NTT
    // =========================================================================
    wire [R*WWIDTH-1:0]  modmul_ntt_in;
    generate
        for (gi = 0; gi < R; gi = gi + 1) begin : gen_d1ton
            wire [WWIDTH-1:0] d1n;
            d1_to_norm #(B) u_d1n (.in(op_lower[gi*WWIDTH +: WWIDTH]), .out(d1n));
            assign modmul_ntt_in[gi*WWIDTH +: WWIDTH] = d1n;
        end
    endgenerate
    ...
    // Pack NTT results into a flat vector for the write mux
    wire [R*WWIDTH-1:0] ntt_result;
    assign ntt_result = {r2ntt_out3, r2ntt_out2, r2ntt_out1, r2ntt_out0};
    ...
    // =========================================================================
    wire [DWIDTH-1:0]    load_word = {data_in_b[WWIDTH-1:0], data_in_a[WWIDTH-1:0]};
    ...
    generate
        for (gi = 0; gi < R; gi = gi + 1) begin : gen_bfly_pack
            ...
            // wr_sel==2'b10 → writing upper half (NTT2); else lower half
            assign bfly_data_pre[gi*DWIDTH +: DWIDTH] = (wr_sel == 2'b10) ?
                {rw, {WWIDTH{1'b0}}} : {{WWIDTH{1'b0}}, rw};
        end
    endgenerate
