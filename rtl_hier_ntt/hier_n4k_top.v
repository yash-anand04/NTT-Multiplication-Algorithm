...
`ifndef _HIER_N4K_TOP_GUARD
`define _HIER_N4K_TOP_GUARD
...
module hier_n4k_top #(
    ...
    always @(*) begin
        ntt_start = 1'b0;
        ntt_inverse = 1'b0;
        // FWD NTT phases (start when issue_valid and state is one of the L_x's)
        case (state)
    ...
    sub_ntt_simple #(.B(B), .L(L)) u_subntt (
        .clk(clk), .rst(rst),
