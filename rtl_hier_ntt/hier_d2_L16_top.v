...
`ifndef _HIER_D2_L16_TOP_GUARD
`define _HIER_D2_L16_TOP_GUARD

module hier_d2_L16_top #(
    parameter B       = 16,
    parameter L       = 16,
    parameter M       = 16,
    ...
    `BMEM_DECL(mem_work)
    `BMEM_DECL(mem_trans)
    // Memory consolidation attempt (2026-05-25): reverted — Phase C's
    // FWD_XTW writes at pos_const_d4 layout but FWD_COL reads at
    // pos_lane_id layout.  Phase D's in-place R/W invariant (read_pattern
    // == write_pattern within a phase) does NOT hold for Phase C and the
    // consolidation broke functional tests (1024 identity errors).
    // Phase C-specific layout would need re-derivation; deferred.
