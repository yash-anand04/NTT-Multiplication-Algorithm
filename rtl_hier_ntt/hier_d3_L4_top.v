...
`ifndef _HIER_D3_L4_TOP_GUARD
`define _HIER_D3_L4_TOP_GUARD

module hier_d3_L4_top #(
    parameter B       = 16,
    parameter L       = 4,
    parameter N       = L * L * L,        // 32768
    parameter WWIDTH  = B + 1,
    parameter LOGN    = 6,               // $clog2(N)
    ...
    localparam integer LOG_DEPTH = 4;

    // -------------------------------------------------------------------------
    // op_count decomposition.  During compute phases:
    //   op_low  = op_count[1:0]   = "scan inner" index
    //   op_high = op_count[3:2]   = "scan outer" index
    ...
    // -------------------------------------------------------------------------
    wire [LOG_LANES-1:0] op_low  = op_count[1:0];
    wire [LOG_LANES-1:0] op_high = op_count[3:2];

    // During LOAD/OUTPUT: op_count = i1 + L*i2 + L^2*i3  (full 15-bit flat index)
    wire [LOG_LANES-1:0] load_i1 = op_count[1:0];
    wire [LOG_LANES-1:0] load_i2 = op_count[3:2];
    wire [LOG_LANES-1:0] load_i3 = op_count[5:4];
    wire [LOG_LANES-1:0] load_bank = (load_i1 + load_i2 + load_i3) & 2'h3;
    ...
    wire [LOG_LANES-1:0] load_i1_d5 = opcnt_d[5][1:0];
    wire [LOG_LANES-1:0] load_i2_d5 = opcnt_d[5][3:2];
    wire [LOG_LANES-1:0] load_i3_d5 = opcnt_d[5][5:4];
    wire [LOG_LANES-1:0] load_bank_d5 = (load_i1_d5 + load_i2_d5 + load_i3_d5) & 2'h3;
    ...
    wire [LOG_LANES-1:0] rshift_op = (op_low + op_high) & 2'h3;
    ...
    generate
        for (gk = 0; gk < LANES; gk = gk + 1) begin : g_rpos
            assign rpos_axis_i3_pack [gk*LOG_DEPTH +: LOG_DEPTH] = {gk[1:0], op_high};
            assign rpos_axis_i2_pack [gk*LOG_DEPTH +: LOG_DEPTH] = {op_high, gk[1:0]};
    ...
    generate
        for (gk = 0; gk < LANES; gk = gk + 1) begin : g_wpos
            assign wpos_i3_d5 [gk*LOG_DEPTH +: LOG_DEPTH] = {gk[1:0], opcnt_d[5][3:2]};
            assign wpos_i3_d8 [gk*LOG_DEPTH +: LOG_DEPTH] = {gk[1:0], opcnt_d[8][3:2]};
            assign wpos_i3_d12[gk*LOG_DEPTH +: LOG_DEPTH] = {gk[1:0], opcnt_d[12][3:2]};
            assign wpos_i2_d5 [gk*LOG_DEPTH +: LOG_DEPTH] = {opcnt_d[5][3:2],  gk[1:0]};
            assign wpos_i2_d8 [gk*LOG_DEPTH +: LOG_DEPTH] = {opcnt_d[8][3:2],  gk[1:0]};
            assign wpos_i2_d12[gk*LOG_DEPTH +: LOG_DEPTH] = {opcnt_d[12][3:2], gk[1:0]};
            assign wpos_bc_d5 [gk*LOG_DEPTH +: LOG_DEPTH] = {opcnt_d[5][3:2],  opcnt_d[5][1:0]};
            assign wpos_bc_d8 [gk*LOG_DEPTH +: LOG_DEPTH] = {opcnt_d[8][3:2],  opcnt_d[8][1:0]};
            assign wpos_bc_d12[gk*LOG_DEPTH +: LOG_DEPTH] = {opcnt_d[12][3:2], opcnt_d[12][1:0]};
        end
    endgenerate

    wire [LOG_LANES-1:0] wshift_d5  = (opcnt_d[5][1:0]  + opcnt_d[5][3:2])  & 2'h3;
    wire [LOG_LANES-1:0] wshift_d8  = (opcnt_d[8][1:0]  + opcnt_d[8][3:2])  & 2'h3;
    wire [LOG_LANES-1:0] wshift_d12 = (opcnt_d[12][1:0] + opcnt_d[12][3:2]) & 2'h3;
    ...
    generate
        for (tg = 0; tg < L; tg = tg + 1) begin : gen_tw_lanes
            ...
            wire [LOG_LANES-1:0] olo_d0 = op_count[1:0];
            wire [LOG_LANES-1:0] ohi_d0 = op_count[3:2];
            ...
            wire [LOG_LANES-1:0] olo_d7 = opcnt_d[7][1:0];
            wire [LOG_LANES-1:0] ohi_d7 = opcnt_d[7][3:2];
