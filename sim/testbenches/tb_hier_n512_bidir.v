// Diagnostic: hier_n512_top with sub_ntt8_bidir (shift-only) substituted in.
// If random_golden passes here → consolidation is the L-variant bug.
// If it fails → sub_ntt8_bidir is incompatible with the d=3 hier.
`timescale 1ns/1ps

module tb_hier_n512_bidir;
    localparam B = 16, L = 8, N = L*L*L, WWIDTH = B + 1;
    ...
    hier_n512_top_bidir_test #(.B(B), .L(L)) dut (
