...
module bivar_ntt_top_v0 #(
    ...
    generate
        for (tg = 0; tg < L; tg = tg + 1) begin : gen_tw_lanes
            ...
            // bivar's FSM expects same-cycle ModMul results, so use the
            // combinational mode (PIPELINED=0). ntt_top uses the 3-stage
            // pipelined default.  Future work: pipeline bivar's data path
            // for a fair Fmax comparison.
